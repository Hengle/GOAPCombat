...
namespace HorrorAgent
{
    ...
    public class Agent : MonoBehaviour
    {
        ...
        public IEnumerator Attack(GameObject target)
        {
            ...
            if (m_agentAttacking != null) //The action is in progress
                yield break;
            m_Animator.SetBool(k_LookAt, false);
            m_Animator.SetBool(k_Chase, false);
            m_Animator.SetBool(k_Attack, true); //setting animator to walk here 
            m_Target = target; //assiging target to the gameobject target 
            m_MotionController.TargetPosition = m_Target.transform.position;

           // Debug.Log("I here 1");
            while (m_Target != null && inAttackDistance())
            {
              //  Debug.Log("I here 2");
                transform.LookAt(target.transform);
                Debug.DrawLine(transform.position, m_MotionController.TargetPosition, Color.green);
                m_MotionController.TargetPosition = m_Target.transform.position;
                ...
                if (!inAttackDistance())
                {
                    ...
                    m_Animator.SetBool(k_Chase, true);
                    m_agentAttacking = null;
                    yield break;
        ...
        bool PlayerSeen()
        {
            ...
            m_Eye = traitHolder.TraitData.FirstOrDefault(t => t.TraitDefinitionName == nameof(EyeSight));
           // Debug.Log("Eyesight: " + m_Eye.GetValue(EyeSight.FieldPlayerSeen));
            ...
            if (targetsInViewRadius.Length >= 1)
            {
                ...
                m_Eye.SetValue(EyeSight.FieldPlayerSeen, true);
                return true;
            }

            else
            {
                m_Eye.SetValue(EyeSight.FieldPlayerSeen, false);
        ...
        {
            var traitHolder = transform.GetComponent<ITraitBasedObjectData>();
            var stamina = traitHolder.TraitData.FirstOrDefault(t => t.TraitDefinitionName == nameof(Stamina));
          //  stamina.SetValue(Stamina.FieldAmount, -5);  --This doesn't work
        ...
        void Update()
        {
            SetStats(); //Need help setting the player stats here 
            PlayerSeen();
            UpdateController();
            //testing to see if we can access and manipulate individual components of the planner outside the planner --Doesn't seem to be the case more testing needed--
        }
