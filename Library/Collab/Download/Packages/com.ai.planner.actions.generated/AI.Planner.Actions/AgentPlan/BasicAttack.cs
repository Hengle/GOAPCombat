...
{
    [BurstCompile]
    struct BasicAttack : IJobParallelForDefer
    {
        ...
        internal BasicAttack(Guid guid, NativeList<StateEntityKey> statesToExpand, StateDataContext stateDataContext)
        {
            ActionGuid = guid;
        ...
        {
            var AgentFilter = new NativeArray<ComponentType>(3, Allocator.Temp){[0] = ComponentType.ReadWrite<AI.Planner.Domains.Agent>(),[1] = ComponentType.ReadWrite<AI.Planner.Domains.Stats>(),[2] = ComponentType.ReadWrite<Unity.AI.Planner.DomainLanguage.TraitBased.Location>(),  };
            var TargetFilter = new NativeArray<ComponentType>(3, Allocator.Temp){[0] = ComponentType.ReadWrite<AI.Planner.Domains.Stats>(),[1] = ComponentType.ReadWrite<AI.Planner.Domains.Player>(),[2] = ComponentType.ReadWrite<Unity.AI.Planner.DomainLanguage.TraitBased.Location>(),  };
            ...
            for (int i1 = 0; i1 < TargetObjectIndices.Length; i1++)
            {
                ...
                var actionKey = new ActionKey(k_MaxArguments) {
                  ...
                  if (!new AttackPrecondition().CheckCustomPrecondition(stateData, actionKey))
                    continue;
        ...
        public void Execute(int jobIndex)
        {
            ...
            var transitionInfo = new NativeArray<BasicAttackFixupReference>(argumentPermutations.Length, Allocator.Temp);
            for (var i = 0; i < argumentPermutations.Length; i++)
            {
                transitionInfo[i] = new BasicAttackFixupReference { TransitionInfo = ApplyEffects(argumentPermutations[i], stateEntityKey) };
            }

            // fixups
            var stateEntity = stateEntityKey.Entity;
            var fixupBuffer = m_StateDataContext.EntityCommandBuffer.AddBuffer<BasicAttackFixupReference>(jobIndex, stateEntity);
    ...
    public struct BasicAttackFixupReference : IBufferElementData
    {
