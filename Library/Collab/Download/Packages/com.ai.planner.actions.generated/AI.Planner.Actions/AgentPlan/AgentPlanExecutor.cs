...
namespace AI.Planner.Actions.AgentPlan
{
    ...
    class AgentPlanExecutor : BasePlanExecutor<TraitBasedObject, StateEntityKey, StateData, StateDataContext, ActionScheduler, DefaultHeuristic, TerminationEvaluator, StateManager, ActionKey, DestroyStatesJobScheduler>
    {
        static Dictionary<Guid, string> s_ActionGuidToNameLookup = new Dictionary<Guid,string>()
        {
            { ActionScheduler.LookAtGuid, nameof(LookAt) },
            { ActionScheduler.ChaseTargetGuid, nameof(ChaseTarget) },
            { ActionScheduler.BasicAttackGuid, nameof(BasicAttack) },
        };
        ...
        public override void Act(DecisionController controller)
        {
            ...
            {
                case var actionGuid when actionGuid == ActionScheduler.LookAtGuid:
                    actionName = nameof(LookAt);
                    break;
                case var actionGuid when actionGuid == ActionScheduler.ChaseTargetGuid:
                    actionName = nameof(ChaseTarget);
                    break;
                case var actionGuid when actionGuid == ActionScheduler.BasicAttackGuid:
                    actionName = nameof(BasicAttack);
                    break;
            }
            ...
            foreach (var argument in argumentMapping)
            {
                ...
                {
                    case nameof(LookAt):
                        parameterIndex = LookAt.GetIndexForParameterName(traitBasedObjectName);
                        break;
                    case nameof(ChaseTarget):
                        parameterIndex = ChaseTarget.GetIndexForParameterName(traitBasedObjectName);
                        break;
                    case nameof(BasicAttack):
                        parameterIndex = BasicAttack.GetIndexForParameterName(traitBasedObjectName);
                        break;
                }
                ...
                if (split.Length > 1)
                {
                    ...
                    {
                        case nameof(Agent):
                            var traitAgent = stateData.GetTraitOnObjectAtIndex<AI.Planner.Domains.Agent>(traitBasedObjectIndex);
                        ...
                        case nameof(Location):
                            ...
                            break;
                        case nameof(EyeSight):
                            var traitEyeSight = stateData.GetTraitOnObjectAtIndex<AI.Planner.Domains.EyeSight>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitEyeSight.GetField(split[2]) : traitEyeSight;
                            break;
                        case nameof(Player):
                            var traitPlayer = stateData.GetTraitOnObjectAtIndex<AI.Planner.Domains.Player>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitPlayer.GetField(split[2]) : traitPlayer;
                            break;
                        case nameof(Stats):
                            var traitStats = stateData.GetTraitOnObjectAtIndex<AI.Planner.Domains.Stats>(traitBasedObjectIndex);
                            arguments[i] = split.Length == 3 ? traitStats.GetField(split[2]) : traitStats;
                            break;
                    }
                }
                else
